...
def bbox_iou(
    ...
    Args:
        ...
        EIoU (bool, optional): If True, calculate Enhanced IoU (optimizes width/height directly).
        eps (float, optional): A small value to avoid division by zero.
    ...
    b2_cx = (b2_x1 + b2_x2) / 2
    b2_cy = (b2_y1 + b2_y2) / 2
    rho2 = (b1_cx - b2_cx).pow(2) + (b1_cy - b2_cy).pow(2)  # center distance squared

    # EIoU: width and height differences
    w_loss = (w1 - w2).pow(2) / (cw.pow(2) + eps)
    h_loss = (h1 - h2).pow(2) / (ch.pow(2) + eps)

    if CIoU or DIoU or GIoU or EIoU:
        if CIoU or DIoU or EIoU:  # Distance or Complete or Enhanced IoU
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)  # CIoU
            elif EIoU:  # Enhanced IoU - directly optimizes width/height
